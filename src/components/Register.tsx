  ...
  const handleGoogleRegister = async () => {
    ...
    try {
      ...
      if (googleUser && googleUser.email) {
        ...
        const userName = googleUser.displayName || cleanEmail.split("@")[0] || "Usuário";
        const cleanStoreName = storeName.trim() || `Loja de ${userName.split(" ")[0]}`;

        const subscription = getDefaultSubscriptionForEmail(cleanEmail, chosenPlanId, true);
        const newUser: User = {
          ...
          registered: true,
          email: cleanEmail,
          phoneNumber: phoneNumber.trim() || "(00) 00000-0000",
          isVip: isVipEmail(cleanEmail),
          subscription: subscription,
        };

        const finalGoal: Goal = invitation ? { targetAmount: 0, period: "Mensal" } : {
          targetAmount: Number(goalAmount || 15000),
        ...
        try {
          ...
          console.warn("sessionStorage Google register save restricted:", e);
        }

        onRegisterComplete(newUser, finalGoal, undefined);
      }
    } catch (err: any) {
      console.warn("Erro no cadastro com Google:", err?.code, err?.message);
      if (err?.code === "auth/popup-closed-by-user") {
        setErrorMessage("Janela de autenticação com o Google foi fechada antes de concluir.");
        ...
        setErrorMessage("Domínio do aplicativo não autorizado no Firebase Console. Adicione este domínio nas configurações de Autenticação do Firebase.");
      } else {
              ...
              {!invitation && (
                <>
                  ...
                  <div className="relative flex py-1 items-center select-none">
                    ...
                    <span className="flex-shrink mx-3 text-[10px] font-sans font-extrabold text-zinc-400 uppercase tracking-widest">
